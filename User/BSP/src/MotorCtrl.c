...
typedef struct _MotorCtrlChannel_s {
    ...
    u16                     PhasePin;
    u8                      Phase;                                              /* phase between logic and actually dir of motor 0: same(default), 1: opposite */
    u16                     SpeedMax;
} MotorCtrlChannel_t;

                                                       /* Chan                  PhaseGPIO   PhasePin        Phase       MaxSpeed        */
static const MotorCtrlChannel_t     MotorCtrlChan[] = {
                                                        { PWM_CHAN_FAN,         NULL,       0,              0,          MOTOR_FAN_CHAN_MAX_SPEED    },              /* Dust cleaner fan     */
                                                        { PWM_CHAN_RWHEEL,      GPIOD,      GPIO_Pin_13,    0,          MOTOR_WHEEL_CHAN_MAX_SPEED  },              /* Left wheel           */
                                                        { PWM_CHAN_LWHEEL,      GPIOE,      GPIO_Pin_5,     1,          MOTOR_WHEEL_CHAN_MAX_SPEED  },              /* Right wheel          */
                                                        { PWM_CHAN_LBRUSH,      NULL,       0,              0,          MOTOR_BRUSH_CHAN_MAX_SPEED  },              /* Left brush           */
                                                        { PWM_CHAN_RBRUSH,      NULL,       0,              0,          MOTOR_BRUSH_CHAN_MAX_SPEED  },              /* Rgith brush          */
                                                        { PWM_CHAN_MBRUSH,      NULL,       0,              0,          MOTOR_BRUSH_CHAN_MAX_SPEED  },              /* Middle brush         */
                                                      };
...
s8 MotorCtrl_Init(void)
{
    ...
    for(i = 0; i < sizeof(MotorCtrlChan)/sizeof(MotorCtrlChan[0]); i++){
        ...
        if(MotorCtrlChan[i].PhasePin != 0 && MotorCtrlChan[i].PhaseGPIO != NULL){
            ...
            GPIO_SetBits(MotorCtrlChan[i].PhaseGPIO, MotorCtrlChan[i].PhasePin);
        }

        if( PWM_CHAN_NULL != MotorCtrlChan[i].PWMResIdx ){
            if( (PWM_CHAN_RWHEEL==MotorCtrlChan[i].PWMResIdx) || (PWM_CHAN_LWHEEL==MotorCtrlChan[i].PWMResIdx)) {
                ret = PWM_ChanInit(MotorCtrlChan[i].PWMResIdx, 0, 0);
            }
            else{
...
s8 MotorCtrl_ChanDirSet(enum MotorCtrlChannel Channel, u8 Dir)
{
    if(MotorCtrlChan[Channel].PhasePin != 0 && MotorCtrlChan[Channel].PhaseGPIO != NULL){
        /* Phase */
        if(MotorCtrlChan[Channel].Phase){
            Dir = !Dir;
        }
...
u8 MotorCtrl_ChanDirGet(enum MotorCtrlChannel Channel)
{
    ...
    Dir = GPIO_ReadOutputDataBit(MotorCtrlChan[Channel].PhaseGPIO, MotorCtrlChan[Channel].PhasePin);
    if(MotorCtrlChan[Channel].Phase){
        Dir = !Dir;
    }
    return (Dir);
